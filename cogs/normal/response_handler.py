...
class ResponseHandler(commands.Cog):
    ...
    async def safe_send(self, ctx, embed=None, content=None, max_retries=5):
        retry_attempts = 0
        delay = 1  # Initial delay in Sekunden
        ...
        while retry_attempts < max_retries:
            try:
                ...
                if e.status == 429:  # Rate limited
                    ...
                    retry_attempts += 1
                    delay *= 2  # Exponentielle Rücknahmestrategie
                else:
        ...
        # If all retries failed, try one last time with a longer delay
        await asyncio.sleep(10)  # Längere Wartezeit vor dem letzten Versuch
        try:
            if embed:
                return await ctx.send(embed=embed)
            else:
    ...
    async def handle_command_response(self, ctx, content):
        # Loading state with safe sending
        ...
        if not message:
            # If we couldn't send the loading embed, try a simple text message
            message = await self.safe_send(ctx, content=f"⏳ Processing: {content}")
            if not message:
                # If we can't send anything, fall back to original ctx.send
                try:
                    message = await ctx.send(f"⏳ Processing: {content}")
                except:
                    return  # If all fails, abort

        await asyncio.sleep(1)

        # Success state with safe editing
        ...
        if not edit_result:
            # If embed edit failed, try simple text edit
