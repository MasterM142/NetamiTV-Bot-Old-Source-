    ...
    def get(self, key: str) -> Optional[discord.Embed]:
        ...
        if key in self.cache:
            ...
            if time.time() - timestamp < self.ttl:
                return embed.copy()  # Return a copy to prevent modifications
            else:
                del self.cache[key]
        return None
...
class EmbedHandler(commands.Cog):
    ...
    def _check_rate_limit(self, ctx_or_interaction) -> tuple[bool, str]:
        ...
        if hasattr(ctx_or_interaction, 'author'):  # Context
    ...
    async def safe_send(self, channel, embed=None, content=None, max_retries=None, **kwargs):
        ...
        while retry_attempts < max_retries:
            try:
                ...
                elif e.status in [400, 403, 404]:  # Client errors that won't be fixed by retrying
                    self.logger.error(f"Client error {e.status}: {e}")
                    self.stats["errors"] += 1
                    raise e
                    
                elif e.status >= 500:  # Server errors, worth retrying
    ...
    async def safe_edit(self, message, embed=None, content=None, max_retries=None, **kwargs):
        ...
        while retry_attempts < max_retries:
            try:
                ...
                elif e.status in [400, 403, 404]:  # Client errors
    ...
    # Create embed methods (without sending)
