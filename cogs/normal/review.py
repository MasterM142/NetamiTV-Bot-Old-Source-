...
REVIEW_CHANNEL_ID =       # Channel wo Admins reviewen
APPROVED_CHANNEL_ID =     # Channel wo genehmigte Bilder gepostet werden  
ADMIN_ROLE_ID =           # Role ID die approve/decline kann
UPLOAD_CHANNEL_ID =       # Channel wo /review verwendet werden darf
USER_ROLE_ID =            # Role ID die /review verwenden darf
# ====================================================
...
class ReviewSystem(commands.Cog):
    def __init__(self, bot):
        ...
        # Anti-Rate-Limit: Channel/User Caching
        self._channel_cache = {}
        self._user_cache = {}
        self._last_request_time = 0
        self._request_delay = 0.1  # Nur 0.1 Sekunden delay für maximale Performance
    ...
    def init_database(self):
        ...
        # Table for tracking who has used /review (one-time-use)
        cursor.execute('''
    ...
    async def get_channel_cached(self, channel_id: int):
        ...
        # Sofort holen ohne delay für maximale Speed
    ...
    async def get_user_cached(self, user_id: int):
        ...
        # Zuerst get_user (instant) versuchen
        user = self.bot.get_user(user_id)
        if not user:
            # Fallback: fetch_user (API call) nur wenn nötig
            try:
    ...
    def has_used_review(self, user_id: int, guild_id: int) -> bool:
        ...
        if self.is_admin_by_id(user_id):
            return False  # Admins can always use it
    ...
    def is_admin_by_id(self, user_id: int) -> bool:
        ...
        # This is a simplified check - in a real scenario you might want to cache this
        # For now, we'll check during the actual command execution
        return False
    ...
    def unlist_user(self, user_id: int, guild_id: int, admin_id: int):
        ...
        # If user wasn't in database, insert them as unlisted (allows usage)
    ...
    def is_user_unlisted(self, user_id: int, guild_id: int) -> bool:
        ...
        # User can use if they were unlisted after their last usage (or never used)
    ...
    async def review_command(self, interaction: discord.Interaction, file: discord.Attachment):
        ...
        # Schnelle Validierung ZUERST (ohne defer für instant response)
        ...
        # Check if user has permission to use /review
        ...
        # Check one-time-use system (nicht für Admins)
        ...
        # SOFORT Bestätigung senden (ohne defer = kein "denkt nach")
        await interaction.response.send_message(
            "✅ Deine Datei wird zur Überprüfung eingereicht...", 
            ephemeral=True
        )
        
        # Alles andere als Background Task (kein defer nötig)
    ...
    async def _process_review_submission(self, user, file, guild_id, channel):
        ...
        try:
            ...
            # Mark user as having used /review (one-time-use für normale User)
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        # Check if it's in the review channel
        ...
        if str(reaction.emoji) == "✅":  # Approved
            await self.handle_approval(reaction.message, submission, user)
        elif str(reaction.emoji) == "❌":  # Declined
    ...
    async def handle_approval(self, message: discord.Message, submission: Dict[str, Any], reviewer: discord.Member):
        ...
        if self.approved_channel_id:
            ...
            if approved_channel:
                ...
                # Sofort posten ohne delay für Speed
                await approved_channel.send(embed=approved_embed)
        
        # Update review message (sofort ohne delay)
        ...
        # Notify user per DM (cached, async background task für Speed)
    ...
    async def handle_decline(self, message: discord.Message, submission: Dict[str, Any], reviewer: discord.Member):
        ...
        # Notify user per DM (async background task für Speed)
    ...
    async def on_message(self, message):
        ...
        # Check if message is in upload channel
        ...
        # Allow messages from users with review permission (only if it's a slash command)
        if isinstance(message.author, discord.Member) and self.can_use_review(message.author):
            # If it's not a slash command, delete it (sofort ohne delay)
        ...
        # Delete all other messages (sofort ohne delay)
        try:
            await message.delete()
        except:
            pass
